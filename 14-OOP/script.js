...
/*
 What is Object-Oriented Programming?
   ...
   - We use objects to model (describe) real-world or abstract features (HTML componenet or data structure).
   ...
   - Interactions happen through a public interface (API): methods that the code outside of the object can acess and use to communicate with the object.
...
/*
 How does OOP actually work in JS?
   ...
   Prototypal inheritance: The prototype object contains methods (behavior) and properties that are accessible to all objects linked to that prototype. 
     ...
     COMPARISON: Prototypal inheritance is DIFFERENT from the 'inheritence' principle on class implementation learned in the previous lecture. In classical OOP with classes, the behaviors (methods) are actually copied from the class to the object and so that is completely different. So that was one class inheriting from another class. But in this case, it's basically an instance inheriting from a class.
 ...
 In practice, how do we implement OOP?
   ...
   - "Syntactic surgar": behind the scenes, ES6 classes work exactly like constuctor functions. (just a nicer syntax)
...
// 'cristian' is an instance of 'Person'
